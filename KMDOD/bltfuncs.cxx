    ...
    _In_reads_(NumRects) CONST RECT *pRects)
{
    ...
    for (UINT iRect = 0; iRect < NumRects; iRect++)
    {
        ...
        BYTE* pStartDst = ((BYTE*)pDst->pBits +
                          ((LONGLONG)pRect->top + pDst->Offset.y) * pDst->Pitch +
                          ((LONGLONG)pRect->left + pDst->Offset.x) * 4);
        CONST BYTE* pStartSrc = ((BYTE*)pSrc->pBits +
                                ((LONGLONG)pRect->top + pSrc->Offset.y) * pSrc->Pitch +
                                ((LONGLONG)pRect->left + pSrc->Offset.x) * 4);
...
BYTE* GetRowStart(_In_ CONST BLT_INFO* pBltInfo, CONST RECT* pRect)
{
    ...
    switch (pBltInfo->Rotation)
    {
        case D3DKMDT_VPPR_IDENTITY:
        {
            pRet = (BYTE*)((LONGLONG)pBltInfo->pBits +
                (LONGLONG)OffTop * pBltInfo->Pitch +
                (LONGLONG)OffLeft * BytesPerPixel);
            break;
        }
        case D3DKMDT_VPPR_ROTATE90:
        {
            pRet = (BYTE*)((LONGLONG)pBltInfo->pBits +
                ((LONGLONG)pBltInfo->Height - 1 - OffLeft) * pBltInfo->Pitch +
                (LONGLONG)OffTop * BytesPerPixel);
            break;
        }
        case D3DKMDT_VPPR_ROTATE180:
        {
            pRet = (BYTE*)((LONGLONG)pBltInfo->pBits +
                ((LONGLONG)pBltInfo->Height - 1 - OffTop) * pBltInfo->Pitch +
                ((LONGLONG)pBltInfo->Width - 1 - OffLeft) * BytesPerPixel);
            break;
        }
        case D3DKMDT_VPPR_ROTATE270:
        {
            pRet = (BYTE*)((LONGLONG)pBltInfo->pBits +
                (LONGLONG)OffLeft * pBltInfo->Pitch +
                ((LONGLONG)pBltInfo->Width - 1 - OffTop) * BytesPerPixel);
            break;
        }
        default:
        {
