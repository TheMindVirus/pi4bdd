...
{
    ////debug("[CALL]: BOOLEAN SynchronizeVidSchNotifyInterrupt");
    ...
    switch(pParam->NotifyInterrupt.InterruptType)
    {
        ...
        break;
        default:
            ////debug("[WARN]: Unknown DXGK_INTERRUPT_TYPE (%d)", pParam->NotifyInterrupt.InterruptType);
            NT_ASSERT(FALSE);
            return FALSE;
    }
...
--*/
{
    ...
    */// ^^ more utter nonsense causing a bugcheck
    /*
    NTSTATUS status = pDevExt->GetDxgkInterface()->DxgkCbSynchronizeExecution( //Returns STATUS_UNSUCCESSFUL and SynchronizeVidSchNotifyInterrupt not called. Interrupt not connected?
        pDevExt->GetDxgkInterface()->DeviceHandle,
        (PKSYNCHRONIZE_ROUTINE)SynchronizeVidSchNotifyInterrupt,
        (PVOID)&SyncNotifyInterrupt, 0, &bRet);
    ...
    */
    ...
    SyncNotifyInterrupt.DxgkInterface->DxgkCbNotifyInterrupt(SyncNotifyInterrupt.DxgkInterface->DeviceHandle, &SyncNotifyInterrupt.NotifyInterrupt);
...
void HwExecutePresentDisplayOnly(HANDLE Context)
{
    ...
    else
    {
        ...
        ReportPresentProgress(ctx->hAdapter,ctx->SourceID,TRUE); ///!!!ReportPresentProgress Bug - Verdict: The OS Requires a Progress Report Interrupt to work properly
    }
